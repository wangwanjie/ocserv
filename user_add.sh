...
function input_user() {
	...
	if  [ ! -n "$user_name" ] ;then
		...
		read -p "Input your vpn user group:" user_group
	fi
	if  [ ! -n "$user_group" ] ;then
    	echo "You did not enter your user_group, will use default group defined in conf"
...
function cert_add() {
	...
	echo "VPN address and port is $public_ip:8888"
}
